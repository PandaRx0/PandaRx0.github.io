...
# 1. Remove subtopics from TOPICS using lines processing
lines = text.split('\\n')
new_lines = []
for line in lines:
    if "id: 'hyperthyroid'" in line or "id: 'thyroid_storm'" in line or "id: 'hypothyroid'" in line:
        continue
    new_lines.append(line)

text = '\\n'.join(new_lines)

# 2. Add MCQs to 'thyroid' property in MCQS object.
end_idx = text.rfind('  },')
...
# Insert thyroid MCQs
thyroid_str = f"\\n  'thyroid': {questions_json},"
text = text[:insert_idx] + thyroid_str + text[insert_idx:]
